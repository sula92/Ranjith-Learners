...
public interface StudentResultRepository extends JpaRepository<StudentResult, String> {
    ...
    @Query(value = "select sr.exam_id as id, ex.date, ex.time, ex.venue,  count(sr.student_id) as totalParticipants, COUNT(CASE WHEN sr.examResult = 'PASS' THEN examResult END) as totalPass, COUNT(CASE WHEN sr.examResult = 'FAIL' THEN examResult END) as totalFail from results sr INNER JOIN exams ex ON sr.exam_id=ex.id where sr.exam_id=:eid", nativeQuery = true)
    List<ExamReport> getExamReportById(@Param("eid") String eid);
